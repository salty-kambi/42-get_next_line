...
/*   Updated: 2021/11/14 20:38:25 by lsuau            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	*line = 0;
	if (BUFFER_SIZE < 1 || fd < 0 || fd > OPEN_MAX)
		return (0);
	if (ft_buffcheck(buff[fd], line))
		return (*line);
	x = read(fd, buff[fd], BUFFER_SIZE);
	while (x > 0)
	{
		buff[fd][x] = 0;
		if (ft_check_newline(buff[fd], line))
		{
			ft_memorage(line, buff[fd]);
			return (*line);
		}
		x = read(fd, buff[fd], BUFFER_SIZE);
	}
